...
export const validateGroupCreationPermission = async (
  ...
  for (const targetGroup of targetGroups) {
    ...
    if (hasSA) {
      ...
      // SPA_yyy can create users with their own SPA_yyy or any TA_XXX group
      if (targetGroup === spaRole || availableTAGroups.includes(targetGroup)) {
        isAuthorized = true;
      } else {
        errorMessage = `${spaRole} cannot create user with group: ${targetGroup}. Allowed groups: ${spaRole}, ${availableTAGroups.join(", ")}`;
      }
...
export const getAvailableTAGroupsForRole = async (
  ...
  if (hasSA) {
    ...
    // SPA_yyy can create users with their own SPA_yyy or any TA_XXX group
    return { groups: [spaRole, ...availableTAGroups] };
  } else if (requesterTARoles.length > 0) {
    // TA_XXX users can create users with any of their TA_XXX groups
