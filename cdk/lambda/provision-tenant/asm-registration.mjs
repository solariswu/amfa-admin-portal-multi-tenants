...
export async function registerOrReuseTenant(tenantData) {
  ...
  try {
    ...
    console.log(`[ASM] Tenant ${tenantId} already registered, reusing existing data`);
    
    // Validate that we have the required fields
    if (!secretData.registRes?.asmClientId || !secretData.registRes?.mobileTokenKey) {
      console.warn(`[ASM] Existing secret missing required fields, re-registering`);
  ...
  const formData = new URLSearchParams({
    ...
    asmTenantInstallerEmail: process.env.INSTALLER_EMAIL || process.env.ADMIN_EMAIL
  });
  
  console.log(`[ASM] Calling ASM API: ${asmPortalUrl}/newTenantAssignmentWithDefaults.ap`);
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (retryCount >= maxRetries) {
        console.error(`[ASM] Registration failed after ${maxRetries} attempts:`, error);
        throw new Error(`Failed to register with ASM portal: ${error.message}`);
      }
      
      console.warn(`[ASM] Registration attempt ${retryCount} failed, retrying...`);
  ...
  try {
    ...
    console.log(`[ASM] Registration data saved to Secrets Manager for ${tenantId}`);
    
  } catch (error) {
    console.error(`[ASM] Failed to save registration to Secrets Manager:`, error);
    // Don't fail the provisioning if secret storage fails - we have the data
    console.warn(`[ASM] Continuing without storing secret (data will be in response)`);
  }
