...
import { getOrganization } from '../organizationslist/crud.mjs';
...
export async function createTenant(data, requesterRole, requesterOrgId) {
  ...
  if (!requesterRole || (requesterRole !== 'SA' && requesterRole !== 'SPA')) {
    throw new Error('Insufficient permissions. Only SA and SPA can create tenants.');
  }
  
  // 2. Validate org authorization for SPA
  if (requesterRole === 'SPA') {
    if (data.orgId !== requesterOrgId) {
      throw new Error(`SPA can only create tenants for their own org (${requesterOrgId})`);
    }
  }
  
  // 3. Validate organization exists
  const tableName = `amfa-${process.env.ACCOUNT}-${process.env.AWS_REGION}-tenanttable`;
  const org = await getOrganization(data.orgId, dynamodb, tableName);
  
  if (!org) {
    throw new Error(`Organization '${data.orgId}' does not exist. Please create it first.`);
  }
  ...
  if (responsePayload.statusCode !== 200) {
    const errorBody = typeof responsePayload.body === 'string' 
      ? JSON.parse(responsePayload.body)
      : responsePayload.body;
    throw new Error(errorBody.message || 'Tenant provisioning failed');
  }
  
  const provisionedTenant = typeof responsePayload.body === 'string'
    ? JSON.parse(responsePayload.body)
    : responsePayload.body;
  ...
  if (requesterRole === 'SA' && data.adminEmail && provisionedTenant.userPoolId) {
    try {
