...
export const getResData = async (item, cognitoISP) => {
  ...
  const email_verified = item.Attributes.find(
    (attr) => attr.Name === "email_verified",
  )?.Value;
  const phone_number_verified = item.Attributes.find(
    (attr) => attr.Name === "phone_number_verified",
  )?.Value;
  const preferredMfaSetting = item.PreferredMfaSetting;
  const hasMobileToken = item.Attributes.find(
    (attr) => attr.Name === "custom:totp-label",
  )?.Value;
  ...
  return {
    ...
    phone_number: item.Attributes.find((attr) => attr.Name === "phone_number")
      ?.Value,
    phone_number_verified: phone_number_verified === "true" ? true : false,
    locale: item.Attributes.find((attr) => attr.Name === "locale")?.Value,
    sub: item.sub
      ? item.sub
      : item.Attributes.find((attr) => attr.Name === "sub")?.Value,
    groups: groups ? groups : null,
    profile: item.Attributes.find((attr) => attr.Name === "profile")?.Value,
    given_name: item.Attributes.find((attr) => attr.Name === "given_name")
      ?.Value,
    family_name: item.Attributes.find((attr) => attr.Name === "family_name")
      ?.Value,
    ...
    totp_label: item.Attributes.find((attr) => attr.Name === "totp_label")
      ?.Value,
    "alter-email": item.Attributes.find(
      (attr) => attr.Name === "custom:alter-email",
    )?.Value,
    "voice-number": item.Attributes.find(
      (attr) => attr.Name === "custom:voice-number",
    )?.Value,
