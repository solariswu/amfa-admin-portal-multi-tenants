...
async function migrateTenantsToDefaultOrg() {
  ...
  const tableName = `amfa-${accountId}-${region}-tenanttable`;
  ...
  try {
    ...
    if (error.name === 'ResourceNotFoundException') {
      console.error('Table does not exist. Please check the table name and region.');
    }
    throw error;
  }
  ...
  const tenantsNeedingMigration = scanResult.Items.filter(item => !item.org_id);
  ...
  if (failCount > 0) {
    console.log('⚠️  Some tenants failed to migrate. Please check the errors above.');
    process.exit(1);
  } else {
